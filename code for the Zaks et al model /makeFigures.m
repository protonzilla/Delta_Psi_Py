...
function makeFigures( samplepam)

%% Import and Plot Experimental Data 
havePamData=1
if havePamData==1
% Move to folder with real PAM traces from plants  and import the data
cd /Users/jzaks/Documents/GradSchool/FlemingLab/NPQ/RealData/Arabidopsis/1201alldata
% load data into class < pamtraceAv.html |pamtraceAv|> 
a=pamtraceAv('100uE')
b=pamtraceAv('500uE')
c=pamtraceAv('1000uE')

%%
% Plot the data for 100 and 1000 micromol photons/ m^2/s. 
%%
a.plotdiff(  25, 'ks-')
c.plotdiff( 27, 'ks-')

% create names for the figures for saving
f25name='npq100uE'
f27name='npq1000uE'
% go back to folder containing code for model
...
quenchyes=2; %index of column in variable samplepam that has quenching 
...
figure(13)
hold on
plotPAM(samplepam{3,quenchyes}, 13 , 'k-', 0)
plotPAM(samplepam{3,1}, 13 , 'r-', offset)
...
plotNPQfromPAM(samplepam{1,quenchyes}, 25, 'b--')
plotNPQfromPAM(samplepam{3,quenchyes}, 27, 'b--')
...
print('-f25', '-depsc', [figfolder f25name])
print('-f27', '-depsc', [figfolder f27name])
